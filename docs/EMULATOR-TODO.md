# Emulator phase, complete

The emulator phase is **done**: on-device signing works end-to-end through the genuine Ledger Device Management Kit on the Speculos emulator. The software core is complete too, **41 tests passing, `tsc` clean**, full pipeline (watcher → policy → settler → audit). The walkthrough video is rendered (`docs/harbormaster-demo.mp4`) and the repo is public; the only remaining build step is optional funding for a real on-chain broadcast.
...
2. **Post** the article and **file** the Google Form (both tracked in `docs/SUBMISSION.md`). The walkthrough video (`docs/harbormaster-demo.mp4`) is ready to upload, and the repo is already public.
