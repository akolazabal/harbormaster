...
3. **`speculos.ts` (`speculos`)** — the **Device Management Kit (DMK)** path over `@ledgerhq/hw-transport-node-speculos` and `@ledgerhq/hw-app-eth`. It opens the Speculos transport, derives the device address at `44'/60'/0'/0/0`, builds an EIP-1559 transaction (estimating nonce/fees/gas via viem against the Base Sepolia RPC), serializes it, requests a clear-signing **resolution**, signs on the device, and broadcasts the signed transaction. A device cancel (APDU `0x6985`) maps to `REJECTED`.

This three-tier shape is how the build satisfies the contest's "Both" answer (Wallet CLI **and** DMK) while staying resilient: if one signing path is unavailable on a given machine, another can carry the proof.
